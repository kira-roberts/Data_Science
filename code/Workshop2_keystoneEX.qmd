...
### Metadata
This metadata file primarily gives us information on the sites. It  shows the specific longitude and latitude of each site, as well as gives it a zone based on where the site lies in the estuary (downstream, marine, etc.). The site names are used across the other datasets and need to be standardized across them all. 
```{r cleanmeta}
...
### The Catch Data
This is our primary data set, it shows the frequency of the fish caught, as well as when and where they were caught. To be able to compare the data with the other data sets we need to ensure consisitancy. 
```{r cleancatch}
...
### Species Information
Our catch data only used the common name to identify the species, however, in scientific publications we need to use the proper scientific nomenclature. This species dictionary allows us to link the common names with the scientific names, but we need to make sure the common names are standardized to what is in the catch data to link them
```{r cleandictionary}
...
The raw_sonde_data is the environmental data collected from the sonde sensor. Like most raw datasets it needs to be cleaned up to ensure usability and some columns need to formatted correctly for the data they contain. As the data will need to be combined with all the other data sets we also need to ensure it is standardized. 
```{r cleansonde}
...
## Statistical Summary
To easily display the patterns in the data that we see, a summary table is recommended. The following code produces one that show the mean, standard deviation  and standard error of the fish counts and salinity for each species per zone. Salinity was chosen as the primary environmental variant as the turbidity was highly variable across all variants (time, site) and temperature was more closely related to the time of day than the site. 
...
## Plotting the Data
Now that we have the data sorted out, the final step is to produce a multi-panel plot (faceted by species) showing fish abundance along the salinity gradient, formatted to professional publication standards. This figure shows not only the statistical summaries but it also shows the individual points 
